...
static inline void add_byte(lzws_code_fast_t* code_ptr, uint_fast8_t* code_bits_ptr, uint_fast8_t source_remainder_bits, uint_fast8_t byte, bool msb) {
...
lzws_result_t lzws_decompressor_read_first_code(lzws_decompressor_state_t* state_ptr, uint8_t** source_ptr, size_t* source_length_ptr) {
  ...
  while (source_bytes != 1) {
    ...
    add_byte(&code, &code_bits, 0, byte, msb);
    source_bytes--;
  }
