...
static inline size_t get_next_code_index(lzws_compressor_dictionary_t* dictionary_ptr, lzws_code_fast_t current_code, uint_fast8_t next_symbol) {
...
lzws_result_t lzws_compressor_allocate_dictionary(lzws_compressor_dictionary_t* dictionary_ptr, size_t total_codes_length, bool block_mode) {
  size_t      codes_length        = get_next_codes_length(dictionary_ptr, total_codes_length);
  ...
  if (block_mode) {
    ...
    lzws_compressor_dictionary_index_t* used_indexes = malloc(used_indexes_length * sizeof(lzws_compressor_dictionary_index_t));
...
void lzws_compressor_clear_dictionary(lzws_compressor_dictionary_t* dictionary_ptr, size_t total_codes_length) {
  ...
  for (size_t index_of_used_index = 0; index_of_used_index < used_indexes_length; index_of_used_index++) {
    lzws_compressor_dictionary_index_t used_index = used_indexes[index_of_used_index];
...
lzws_code_fast_t lzws_compressor_get_next_code_from_dictionary(lzws_compressor_dictionary_t* dictionary_ptr, lzws_code_fast_t current_code, uint_fast8_t next_symbol) {
  size_t code_index = get_next_code_index(dictionary_ptr, current_code, next_symbol);
...
void lzws_compressor_save_next_code_to_dictionary(
  ...
  if (block_mode) {
    size_t index_of_used_index                      = get_index_of_used_index(dictionary_ptr, next_code);
    dictionary_ptr->next_codes[index_of_used_index] = code_index;
  }
